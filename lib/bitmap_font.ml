...
let is_hangul charcode = charcode >= 0xac00 && charcode <= 0xd7a3
let is_ascii charcode = charcode >= 0x0000 && charcode <= 0x007f
...
let draw_ascii renderer texture charcode x y =
  ...
  let src_rect =
    Sdl.Rect.create ~x:(asc_col * ascii_width) ~y:(asc_row * ascii_height)
      ~w:ascii_width ~h:ascii_height
  in
  let dst_rect = Sdl.Rect.create ~x ~y ~w:ascii_width ~h:ascii_height in
  ignore
    (Sdl.render_copy_ex ~src:src_rect ~dst:dst_rect renderer texture 0.0 None
       Sdl.Flip.none)
